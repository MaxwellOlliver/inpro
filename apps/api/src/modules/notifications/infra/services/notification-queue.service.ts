import { Err, Ok, PlainAggregate } from '@inpro/core';
...
export class NotificationQueueService implements INotificationQueueService {
  ...
  private makeNotification(
    ...
    if (notificationResult.isErr()) {
      return Err(notificationResult.unwrapErr());
    }
  ...
  async processNotification(
    ...
    if (sendResult.isErr()) {
      notification.markAsFailed(sendResult.unwrapErr().message);
      await this.notificationRepository.save(notification);

      return Err(sendResult.unwrapErr());
    }
